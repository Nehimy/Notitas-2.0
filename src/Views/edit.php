...
?>	
		...
		<div id="sky-blue-set">
		  <div class="transparent-set">
			  <div class="white-set">
				  <div class="new-litle-note <?=$view->editMyNota->colour?>">
					  <form method="POST" action="<?=SITE_URL?>note/<?=$view->editMyNota->id?>/update">
						  ...
						  <?php
						    ...
						    $selected = $view->editMyNota->colour;
						  ...
						  ?>
						  <select class="<?=$view->editMyNota->colour?>" name="color" onChange="changeColor(this)">
						    <?=$selected?>
                <?=$notSelected?>
						  <!--
